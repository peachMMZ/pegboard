import { PegboardItem, PegboardItemType } from '@/store/pegboard'
...
export const widgetComponentMap: Record<PegboardItemType, Component> = {} as any
export const widgetMetaMap: Record<PegboardItemType, WidgetMeta> = {} as any

for (const path in plugins) {
  const module = plugins[path]
  const plugin = module.default
  widgetComponentMap[plugin.type] = plugin.component
  widgetMetaMap[plugin.type] = plugin.meta
}
...
export function resolveWidgetComponent(item: PegboardItem) {
  ...
  return widgetComponentMap[item.type]
}
